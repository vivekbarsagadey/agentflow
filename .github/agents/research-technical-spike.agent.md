...
tools: ['edit', 'runNotebooks', 'search', 'new', 'runCommands', 'runTasks', 'github/github-mcp-server/*', 'microsoft/markitdown/*', 'microsoft/playwright-mcp/*', 'microsoftdocs/mcp/*', 'usages', 'vscodeAPI', 'problems', 'changes', 'testFailure', 'openSimpleBrowser', 'fetch', 'githubRepo', 'github.vscode-pull-request-github/activePullRequest', 'github.vscode-pull-request-github/openPullRequest', 'extensions', 'todos', 'runSubagent']
handoffs:
  - label: Create Implementation Plan
    agent: implementation-plan
    prompt: Create a structured implementation plan based on the technical spike research findings.
    send: false
  - label: Create Task Plan
    agent: task-planner
    prompt: Break down the implementation into actionable tasks based on the spike research.
    send: false
  - label: Start Implementation
    agent: expert-python-fastapi-engineer
    prompt: Begin implementing the solution validated by the technical spike research using AgentFlow FastAPI/LangGraph best practices.
    send: false
---
...
✅ Technical spike is complete when:
- All research questions from spike document addressed
- Findings verified across multiple tools and sources
- Experimental validation performed (with user permission)
- Spike document continuously updated throughout research
- Investigation Results section contains detailed findings with evidence
- External Resources section includes all sources with context
- Prototype/Testing Notes document experimental results
- Decision/Recommendation provides clear conclusion
- Status History marked complete
- All todos marked complete or have clear next steps
- Research chronology clearly documented
- Both successful findings AND dead ends documented
